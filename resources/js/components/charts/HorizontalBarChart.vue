    ...
    drawDiagram() {
      ...
      const column1width =
        window.innerWidth > SMALL_BREAKPOINT
          ? PREFFERED_COLUMN_WIDTH.LARGE
          : PREFFERED_COLUMN_WIDTH.SMALL;
      ...
      const handleSortClick = (headerEl, sortField) => {
        ...
        diagramData = diagramData
          .slice()
          .sort((a, b) =>
            d3[header.attr("aria-sort")](a[sortField], b[sortField])
          );
        sortRows();
      };
      ...
      tr.append("td")
        ...
        .text(function (d) {
          return self.multilang
            ? extractLabelForSelectedLanguage(d.title, self.lang)
            : d.title;
        });
